...
- 2026-07-03: Catálogo dinámico de apps instaladas (pedido por el usuario: abrir WhatsApp
  de escritorio, juegos como Red Dead Redemption, etc. — `APPS_CONOCIDAS` solo cubría 10
  apps y "whatsapp" abría la versión web). Se evaluó agregar las apps a mano a
  `APPS_CONOCIDAS` vs. leer el catálogo del sistema; **decisión del usuario: catálogo
  automático**, por dos razones: (1) cero mantenimiento (cualquier app que instale a
  futuro funciona sola), (2) los AppID de juegos son auto-generados
  (`Microsoft.AutoGenerated.{...}`) y cambian al reinstalar — hardcodearlos se rompe en
  silencio. Implementación en `herramientas.py`: `_obtener_catalogo_apps()` ejecuta UNA
  vez por arranque `Get-StartApps | ConvertTo-Json` vía PowerShell (devuelve nombre →
  AppID de TODO el menú Inicio: apps UWP de la Store, juegos, launchers, .exe clásicos;
  ~194 apps en la PC del usuario, ~1-2s) y cachea el dict con `threading.Lock`;
  `precargar_catalogo_apps()` lo carga en un hilo daemon al arrancar `asistente.py`
  (en paralelo con la carga de Whisper/Piper, así el primer "abrí X" no espera).
  `abrir_app` busca en 3 niveles: atajos manuales de `APPS_CONOCIDAS` (prioridad) →
  match exacto en el catálogo → substring (prefiriendo el nombre más corto) y
  `difflib.get_close_matches` (cutoff 0.75) para errores fonéticos del STT. Los nombres
  se normalizan sin acentos (`unicodedata`), así "wásap"/"wasap" encuentra "WhatsApp".
  El lanzamiento es `os.startfile("shell:AppsFolder\\<AppID>")`, que funciona igual para
  UWP y accesos clásicos. "whatsapp" se quitó de `WEBS_CONOCIDAS` (ahora abre la app de
  escritorio, no la web). Verificado: "wasap" y "red det redemption" (errores fonéticos)
  resuelven a las apps correctas, WhatsApp abre de verdad, app inexistente devuelve el
  mensaje de error sin excepción, y si PowerShell falla se cachea un catálogo vacío y
  `abrir_app` sigue funcionando con los atajos manuales.
- 2026-07-03: Nueva herramienta `cerrar_app(nombre, forzar)` (pedida por el usuario tras
  aprobar el catálogo de apertura). Cerrar es distinto a abrir: hay que encontrar el
  PROCESO en ejecución, y verificado en su PC (a) el nombre de proceso no coincide con
  el hablado (Red Dead Redemption corre como proceso `RDR`; su título de ventana sí dice
  "Red Dead Redemption") y (b) matar por NOMBRE de proceso es peligroso: todas las apps
  de la Store viven dentro de UN `ApplicationFrameHost` y WhatsApp usa un
  `msedgewebview2` compartido con otras apps. Decisiones: (1) se listan solo procesos
  con ventana visible (`Get-Process | Where MainWindowTitle`, en vivo, sin cache — lo
  abierto cambia todo el tiempo); (2) matcheo difuso reutilizando el helper extraído
  `_mejor_coincidencia()` (el mismo de `_buscar_en_catalogo`), primero contra nombres de
  proceso y como respaldo contra títulos de ventana (los títulos traen ruido tipo
  "archivo.py - Cursor", matchearlos primero daría falsos positivos); (3) cierre amable
  POR PID con `CloseMainWindow()` (= clic en la X: la app puede preguntar "¿guardar?"),
  nunca `taskkill /IM`; (4) `forzar=true` (kill con `psutil.terminate()`) solo si el
  usuario lo pide explícitamente tras fallar el cierre amable — mismo patrón de dos
  pasos que `apagar_o_suspender_pc` — con blocklist `PROCESOS_PROTEGIDOS` (`explorer`,
  `ApplicationFrameHost`, `TextInputHost`). Hallazgos al probar con WhatsApp real:
  (a) al cerrar la ventana, WhatsApp queda vivo en la bandeja del sistema → el éxito se
  verifica mirando si la VENTANA desapareció (no si el proceso murió), porque para el
  usuario eso ES cerrarla; (b) por lo mismo, `forzar` tiene un respaldo por nombre de
  proceso (`_forzar_cierre_por_proceso`, matcheo estricto sin difflib: matar por
  parecido podría tumbar un proceso ajeno) para apps que ya no tienen ventana pero
  siguen en la bandeja. `cerrar_app` agregada a `HERRAMIENTAS_NO_REPETIBLES_POR_TURNO`
  en `llm.py`. Verificado: round-trip abrir/cerrar bloc de notas, WhatsApp (cierre
  amable con doble proceso + forzado desde la bandeja), app inexistente con y sin
  forzar devuelve mensaje sin excepción.

- 2026-07-03: Tercer bug no determinístico de Qwen3 en function calling (reportado por el
  usuario: "Asistente: Abro WhatsApp." y no se abría nada, en TODA la sesión). Causa,
  reproducida con la transcripción real del usuario: a veces el modelo ANUNCIA la acción
  como respuesta final de texto ("Abro WhatsApp.") sin emitir ningún `tool_call` (ni
  estructurado ni en texto plano). Efecto dominó: ese anuncio vacío queda en el
  historial y los turnos siguientes lo imitan, así que una sesión entera puede quedarse
  sin herramientas (por eso "no abría NADA" y no "a veces falla"). Con historial fresco
  la misma frase a veces sí llama la herramienta: es no determinístico (temperature 0.7).
  Mitigación en `llm.py`: (1) regla nueva en `PROMPT_SISTEMA` (nunca anunciar sin llamar);
  (2) `_parece_anuncio_sin_accion()` — regex de verbos de acción ("abro", "cierro",
  "pongo", "voy a abrir", ...) que EXCLUYE preguntas ("¿Querés que abra X?" es un pedido
  de confirmación legítimo, no hay que forzar nada); (3) si la respuesta es un anuncio
  sin acción y todavía no se ejecutó ninguna herramienta en el turno, se reintenta UNA
  vez la misma petición con `tool_choice="required"` (verificado que NIM/Qwen lo
  soporta: obliga al modelo a llamar alguna herramienta), y el anuncio vacío NO se
  guarda en el historial (corta el efecto dominó). La guarda `ejecutadas_este_turno`
  evita falsos positivos: un "Abriendo X." como respuesta final DESPUÉS de ejecutar la
  herramienta real es correcto y no dispara reintento. Verificado re-simulando la sesión
  exacta del usuario (7 turnos con errores fonéticos reales del STT): todos los pedidos
  ejecutan su herramienta, y el modelo hasta interpretó "Abre Opera Re X" → Opera GX.
